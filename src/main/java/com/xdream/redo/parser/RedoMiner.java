...
public class RedoMiner {
    ...
    private static final String SQL_ONLINE = "select CURRENT_SCN,\n" +
            "       L.SEQUENCE#, F.MEMBER, L.BLOCKSIZE, L.BYTES\n" +
            "from   V$DATABASE D, V$LOG L, V$LOGFILE F\n" +
            "where  L.STATUS = 'CURRENT'\n" +
            "  and  L.GROUP# = F.GROUP#\n" +
            "  and  L.THREAD#=1\n" +
            "  and  F.STATUS is null\n" +
            "  and  rownum = 1 and CURRENT_SCN <= ?";

    private static final String SQL_ARCHIVE = "select NAME,SEQUENCE#,BLOCKS,BLOCK_SIZE from SYS.V_$ARCHIVED_LOG where NEXT_CHANGE# > ? order by SEQUENCE#";
    ...
    public void parseRedoFile() {
        ...
        while (true) {
            try {
                List<CurrentRedo> currentRedoList = getCurrentRedoList(lastScn);
    ...
    private List<CurrentRedo> getCurrentRedoList(long Scn) throws SQLException {
        // 先查询归档
